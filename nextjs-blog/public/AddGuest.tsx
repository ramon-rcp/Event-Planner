...
export class AddGuest extends Component<AgProps, AgState> {
    ...
    render = (): JSX.Element => {
        ...
        return <main className={styles.main}>
            ...
            <p>
                Allergies:
                <textarea rows={2} cols={60} value={this.state.allergies} onChange={this.updateAllergies}></textarea>
            </p>
            <p>
                Plus one:
            ...
            {this.renderKids()}
            <button className={styles.buttonContainer} onClick={this.addGuest}>Add</button>
            <button className={styles.buttonContainer} onClick={this.goBack}>Back</button>
    ...
    renderKids = (): JSX.Element => {
        ...
        for (let i: number = 0; i < Number(this.state.kids); i++) {
            html.push( <div className={styles.kidInfo}>
                    <p>
                Kid {i + 1} Name:
                <input type="text" name={i.toString()} onChange={this.updateKidName}
                       value={this.state.kidinfo?.names[i]}></input>
            </p>
                <p>
                    Kid {i + 1} Allergies:
                    <textarea rows={4} cols={60} name={i.toString()} value={this.state.kidinfo?.allergies[i]}
                              onChange={this.updateKidAllergies}></textarea>
                </p></div>)
        }
        return <div>{html}</div>
    }
    ...
    renderPlusOne = (): JSX.Element => {
        ...
        return( <div>
            ...
            <p>
                Plus One Allergies:
                <textarea rows={4} cols={60} value={this.state.plusoneinfo?.allergies}
                          onChange={this.updatePoAllergies}></textarea>
            </p>
        </div> )
    }
